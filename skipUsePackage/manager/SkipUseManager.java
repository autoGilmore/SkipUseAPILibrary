...
public enum SkipUseManager {
    ...
    private final Map<Long, ServerMemberCategoryList> memberIDCategoryListMap = new HashMap<>();
    ...
    public static final String PICK_ID_CSV_DELIMITER = ", ";

    // Manager helper variables
    private final PickIDCountAdvanceList pickIDCountUpdates = new PickIDCountAdvanceList();
    ...
    public void login(String email, String password) throws SkipUseException {
        try {
            // try to log out in order to reset session data if needed
            logout();
    ...
    // This will also delete the member's Picks and categories.
    ...
    public void deleteMemberByID(long memberID) throws SkipUseException {
        if (getServerMemberMap().getMemberIDMap().containsValue(memberID)) {
            ...
            // reset the member ID map so that a re-load will be triggered.
    ...
    public void setStopUsingByMemberIDPickIDTrueFalse(long memberID, String pickID, boolean stopUsing)
            throws SkipUseException {
        // find the Pick, do not provide a member collection (use the current collection)
        Pick _foundPick = service._getPickByMemberIDAndPickIDAndCollectionID(memberID, pickID, 0L);
