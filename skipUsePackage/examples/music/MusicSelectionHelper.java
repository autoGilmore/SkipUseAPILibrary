...
public enum MusicSelectionHelper {
	...
	private Set<String> memberCategorySet = new HashSet<>();

	// Play-back and selection variables.
	private List<String> playerSearchOptionList = new ArrayList<>();
	private List<ResultOption> playerResultOptionList = new ArrayList<>();

	private List<String> selectedCategoryList = new ArrayList<>();
	private List<String> lastSelectedCategoryList = new ArrayList<>();
	...
	private Map<String, LocalDate> getPickIDHistoryMap = new HashMap<>();
	private Queue<Pick> pickQueue = new LinkedList<>();
	...
	public void reset() {
		...
		lastSelectedCategoryList = new ArrayList<>();
		isResendPickQuery = true;
		pickQuery = new PickQuery();
		returnedPickList = new ArrayList<>();
		getPickIDHistoryMap = new HashMap<>();
		pickQueue = new LinkedList<>();
	}
	...
	//
	public void setSongIDCollection(List<String> songIDList, boolean isCommaSpaceDelimited) {
		memberCollection = new MemberCollection();
		try {
			syso.logInfo("MusicSelectionHelper " + syso.getLineNumber() + " Try setSongIDCollection  songIDList.size: "
					+ songIDList.size() + " isCommaSpaceDelimited: " + isCommaSpaceDelimited);
			memberCollection = skipUseManager.addMemberCollection(0L, songIDList, isCommaSpaceDelimited);
		} catch (SkipUseException e) {
			syso.logError(syso.getLineNumber() + " MusicSelectionHelper", e.getMessage());
			PlayerStatus.addToErrorMessageQueue(e.getMessage());
			isSkipUseError = true;
	...
	public Pick _getNextPick() {
		...
		while (isFindPick) {
			if (!pickQueue.isEmpty()) {
				...
				// check we have not used this Pick before the time limit
	...
	public void loadPickQueue() {
		...
		for (Pick pick : returnedPickList) {
			...
			if (!pickQueue.contains(pick)) {
				pickQueue.add(pick);
				syso.logInfo("PickIDQueue loading: " + pickID + " for member: " + pick.getMemberID() + " cat: "
						+ pick.getCategoryList());
			}

			// if we get back an owner's Pick, something went wrong
			try {
	...
	public void toggleSongCategory(String songID, long memberID, Category toggleCategory) {
		...
		if (_pick != null) {
			if (_pick.getCategoryList().contains(categoryName)) {
				// un-mark category.
				syso.logInfo("un-mark category: " + categoryName);
	...
	// Set the StopUsing flag for a member Pick
	//
	public void updateMemberPickStopUsingFlag(long memberID, String stopUsingPickID, boolean isStopUsing) {
		try {
			skipUseManager.setStopUsingByMemberIDPickIDTrueFalse(memberID, stopUsingPickID, isStopUsing);
		} catch (SkipUseException e) {
			syso.logError(syso.getLineNumber() + " MusicSelectionHelper", e.getMessage());
			PlayerStatus.addToErrorMessageQueue(e.getMessage());
			isSkipUseError = true;
	...
	//
	public List<String> getPlayerSearchOptionList() {
		return playerSearchOptionList;
	}

	// Set just one search option list.
	//
	public void addOnePlayerSearchOptionList(String searchOption) {
	...
	//
	public void addToPlayerSearchOptionList(String searchOption) {
	...
	//
	public void setPlaySearchOptionList(List<String> playerSearchOptionList) {
		clearPlayerSearchOptionList();
		for (String searchOption : playerSearchOptionList) {
			addToPlayerSearchOptionList(searchOption);
		}
	}
	...
	public List<Long> getListeningMemberIDSet() {
		...
		return new ArrayList<>(listeningMemberIDSet);
	}
	...
	private void removePickFromCurrentPickList(long memberID, String pickID) {
		getCurrentPickList().stream().filter(p -> p.getPickID().equals(pickID) && p.getMemberID() == memberID)
				.findFirst().ifPresent(_foundPick -> getCurrentPickList().remove(_foundPick));
	}
	...
	private void loadPickQuery() {
		...
		if (!getSelectedCategoryList().isEmpty()) {
			...
			if (!hasCategoryOption) {
				...
				pickQuery.getSearchOptionList().remove(SearchOption.GET_MORE_IF_SHORT.name());
			}
		}
	...
	//
	private void markSongIDWithCategoryTrueFalse(long memberID, String songID, String category) {
		try {
